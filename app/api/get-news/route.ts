...
export async function GET() {
  try {
    ...
    const newsResults: any = await sheets.spreadsheets.values.get({
    ...
    if (!newsResults.data.values || newsResults.data.values.length === 0) {
    ...
    const mappedResult = newsResults.data.values.map((item: any) => {
      return {
        id: item[0],
        title: item[1],
        thumbnail1: item[2],
